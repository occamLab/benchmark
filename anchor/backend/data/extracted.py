...
class Extracted:
    ...
    def append_video_timestamp(
        self, timestamp: int, frame_path: Path, frame_num: int, mapping_phase: bool
    ):
        phase = Extracted.get_phase_key(mapping_phase)
        self.sensors_extracted[phase]["video"].append(
            {"timestamp": timestamp, "frame_path": frame_path, "frame_num": frame_num}
        )

    def append_intrinsics_data(
        self,
        timestamp: int,
        fx: float,
        fy: float,
    ...
    def append_google_cloud_anchor_localization(
        self, timestamp, anchor_rotation_matrix: [float], arkit_rotation_matrix: [float]
    ):
        ...
        self.sensors_extracted[phase]["google_cloud_anchor"].append(
            cloud_anchor_detection
        )
    ...
    def match_given_sensor(self, phase: str, match_against: str):
        ...
        ) and sensor_idx < len(self.sensors_extracted[phase][match_against]):
            frame_timestamp = self.sensors_extracted[phase]["video"][frame_idx][
                "timestamp"
            ]
            sensor_timestamp = self.sensors_extracted[phase][match_against][sensor_idx][
                "timestamp"
            ]
        ...
        assert match_count == len(
            self.sensors_extracted[phase]["video"]
        ), "failed to match all video frames to sensor data"
    ...
    def transform_poses_in_global_frame(self):
        ...
        for phase in self.sensors_extracted:
            ...
            true_april_tag_loc = np.reshape(
                self.sensors_extracted[phase]["april_tags"][0]["rotation_matrix"],
                (4, 4),
            ).transpose()
            ...
            for april_tag in self.sensors_extracted[phase]["april_tags"]:
                april_tag_loc = np.reshape(
                    april_tag["rotation_matrix"], (4, 4)
                ).transpose()
                average_pose_translation = np.add(
                    average_pose_translation, april_tag_loc[:, [3]]
                )
            ...
            for idx, pose in enumerate(self.sensors_extracted[phase]["poses"]):
                ...
                relative_to_april = np.matmul(
                    np.linalg.inv(true_april_tag_loc), pose_transform
                )
                original_list_form = relative_to_april.transpose().flatten().tolist()
                self.sensors_extracted[phase]["poses"][idx][
                    "rotation_matrix"
                ] = original_list_form
