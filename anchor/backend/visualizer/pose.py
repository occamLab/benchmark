...
def plot_pyramid(
    vertices: List[Tuple[float, float, float, float]], color: str, axes: plt.Axes
):
    ...
    axes.add_collection3d(
        Poly3DCollection(
            verts, facecolors=color, linewidths=1, edgecolors=color, alpha=0.25
        )
    )
...
def visualize_pose_list():
    ...
    for idx, _ in enumerate(arkit_poses):
        ...
        anchor_to_arkit = (
            anchor_pose @ device_arkit_to_device_opencv @ np.linalg.inv(arkit_pose)
        )
