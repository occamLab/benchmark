...
def featurepoint_extraction(input_images: Path, output_path: Path, num_pair: int):
    ...
    global_descriptors = extract_features.main(
        retrieval_conf, input_images, output_path, overwrite=True
    )
    ...
    features, loc_matches = match_dense.main(
        matcher_conf, image_pairs, input_images, export_dir=output_path, overwrite=True
    )
...
if __name__ == "__main__":
    ...
    extracted_location: Path = downloader.extract_ios_logger_tar(
        tar_firebase_path, tar_file_name
    )
